import type { CoreRule } from '@/components/core-rules/data'
...
export type DiffEntry =
	| { kind: 'added'; rule: CoreRule }
	| { kind: 'removed'; rule: CoreRule }
...
function normalizedText(rule: CoreRule): string {
...
/** Order-preserving alignment of two index ranges under an arbitrary equality predicate. */
...
/**
 * Diff two ordered rule sets by **content**. Rules are first anchored on identical text via an
 * LCS, so rules that were only renumbered between versions align and are omitted entirely. The
 * remaining gaps are re-aligned by same-id-or-similarity: matched rules become modifications
 * (labelled with the new number), the rest are genuine additions/removals. Document order is
 * preserved.
 */
export function diffRuleSets(oldRules: CoreRule[], newRules: CoreRule[]): DiffEntry[] {
	...
	const flushGap = () => {
		...
		const eq = (i: number, j: number) => {
			...
			return oldRule.id === newRule.id || textsMatch(oldText[gapOld[i]], newText[gapNew[j]])
		}
		for (const op of alignIndices(gapOld.length, gapNew.length, eq)) {
			if (op.type === 'remove') {
				...
				// likewise noise (the referenced rule moved, not this one's meaning). Omit both.
				if (oldNorm === newNorm) continue
				if (maskRuleReferences(oldNorm) === maskRuleReferences(newNorm)) continue
	...
	// Anchor on identical text; unchanged (incl. renumbered-only) rules fall on the LCS and are hidden.
	const anchorOps = alignIndices(oldRules.length, newRules.length, (i, j) => oldText[i] === newText[j])
	for (const op of anchorOps) {
		if (op.type === 'same') flushGap()
		else if (op.type === 'remove') gapOld.push(op.oldIndex)
		else gapNew.push(op.newIndex)
	}
	flushGap()
